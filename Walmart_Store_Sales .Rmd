...
\newpage
# Introduction 
  This project is based on a dataset of Walmart weekly sales originally posted on **kaggle.com**. The link is: [https://www.kaggle.com/aditya6196/retail-analysis-with-walmart-data](https://www.kaggle.com/aditya6196/retail-analysis-with-walmart-data). This posted dataset, **Walmart_Store_sales.csv**, contains  all weekly in-store sales of _45_ Walmart stores in _143_ weeks --- totally, _6435_ samples. The time period covers _143_ consecutive weeks from February 5, 2010 to November, 1, 2012. Moreover, the author also proposed a series of task requirements to the purposed competitors in a description file. Since Kaggle.com requires user logon information, I transferred both the data and description files to my Github site. As a result, my program will download the data file automatically from my Github rather than Kaggle.com. 
  
  In this paper, I will present my study pretending as a competitor by following the original requirements. However, I also have to add extra nonlinear data analysis to meet the **HarvardX** course requirement, since the author only asks for linear regression. Finally, because the original description did not mention the **unit** of the sales values, I assume it is in **US Dollars**.  The following table lists the tasks from the author's assignment:
...
For **Statistical Model** part, I removed linear regression requirement to make the task more general. Sequentially, I will address this two parts assignment. First, I will run **basic statistics analysis** in the section of **Data Manipulation and Exploratory Analysis**. Then, **Statistical Model** in **Modeling Data Analysis** section.

# Data Manupilation and Exploratory Analysis

  The data file was provided in csv format having a size of _355 KB_. It contains _6435_ records matching exactly _45_ stores by _143_ weeks. By download, I create a data frame, **dat**, in R to hold all the samples. Its summary is shown below:
...
```
Using the following code, I convert all dates into "**dd-mm-yy**" format. Then, create a column **Day_class** based on **Holiday_Flag** to reflect the holiday names. A new dataset is then created, named **ndat** and the weeks with special and normal days are listed below. Now, we have a dataset ready to proceed for analysis.   
...
### Task 1 and 2: Store with maximum sales and store with maximum standard deviation and its coefficient of mean
  ...
  The figure below shows the **total store sales** with respect to the store number in all _143_ weeks. The sale values look random. Nevertheless, we can easily identify that Store _20_ has the maximum accumulated sales. 
...
labs(title = "Plot of store sales, quater 2&3, 2012", y = "sales", 
     x = "store number", caption="Walmart Stores Sales")
```
  Next, I compute the Q3 growth of **growth** in **Dollar** values and relative **percentage** values in the following chunk of code. The next two side by side figures show the **store sale growth** in terms of total sales and percentage. The green dash lines marks the **zero growth** point. Clearly, the most stores' **sale growth** is negative in Q3, 2012. However, we can figure out that there are **_10_** stores have positive growth. Store **_7_** is the best performer. It has achieved a **_13.3%_** quarterly increase. As a summary, `r 10/45*100` percent stores have a good quarter in Q3, 2012. 
...
# new variables: month, year, quarter
...
```  
Summarizing the _5_ task studies above, I put all the answers in the following table. 


 Table: Answers for basic statistics tasks
...
# Modeling Data Analysis
  ...
  First, let us have a insight look at Store 1 data. The following two side-by side figures show the monthly and quarterly sales of the store. Unlike what we saw above for all stores' case, Store 1 has  positive growth from year to year. The Q4 decrease of _2012_ is only due to missing data.    
...
ndat1%>%group_by(year,quarter)%>%filter(Store==1)%>%
  ...
  labs(title = "Plot of quaterly sales", x="month", y="sale($)", caption="Walmart Stores Sales")
```
  Since our objective to examine the impact of **CPI**, **Unemployment** and **Fuel_Price** to the weekly sales, let us also have quick look at their behaviors. The next _3_ figures shows the corresponding quarterly variation respectively. The **CPI** demonstrates a distinct pattern: it grows year by year. On the other hand, the **Unemployment** and **Fuel_Price** have another type of pattern. Shown in the figure, the change of the **Unemployment rate** is mixed in the first two year and then it decreases dramatically in _2012_. Meanwhile,  the **Fuel price** stays low in the first year and then it increases dramatically in second year and then, there is no clear boundary in second and third years.   

```{r,  cache=TRUE, warning=FALSE, message = FALSE, comment=NA, fig.cap="Count of CPI", fig.width=4, fig.height=3, fig.align="center"}  
...
```{r,  cache=TRUE, warning=FALSE, message = FALSE, comment=NA, fig.cap="Count of Unemployment", fig.width=4, fig.height=3, fig.align="center"}  
...
```{r,  figure-fuel, cache=TRUE, warning=FALSE, message = FALSE, comment=NA, fig.cap="Count of Fuel Price", fig.width=4, fig.height=3, fig.align="center"}  
...
## Data Analysis  
   ...
   First of all, for simplicity, I create a new dataset called **_ndat2_** by choosing the necessary parameters for the analysis as below.
```{r,  cache=TRUE, warning=FALSE, message = FALSE, comment=NA}  
...
### Linear Models
  ...
  where  \({Y}_{c,u,f}\) denotes the weekly sales;  \(\epsilon\) denotes the independent errors;  \(\mu\) is the _mean_ of all _ratings_ of the dataset; \( b_{c}\),  \( b_{u}\) and \( b_{f}\) represent the bias for specific _CPI_, _Un-employment rate_ and _Fuel Price_ respectively. I test the equation step by step by starting from single variable model, \(\mu\).  

#### Model _1_: \(\mu\) only  
 
  Let us consider the simplest model only using \(\mu \), then we run the code below by saving the **RMSE**. The figure below shows the data mismatch between the test set and their prediction ordered by their row index (_x-axis_). Since the prediction is a single value, \(\mu\), the prediction is just a horizontal line (blue).  
```{r,  cache=TRUE, warning=FALSE, message = FALSE, comment=NA}  
# model 1: mu only
...
#### Model _3_: \(\mu\) + CPI + Unemployment 
  ...
  Adding in _Unemployment_ factor, re-run the linear regression, the result is shown below. Unfortunately, the **RMSE** here does not reduce. On the contrast, it becomes higher. Also, the prediction line does not change much shown in the figure below.
...
#### Model _4_: \(\mu\) + CPI + Unemployment + Fuel Price
  ...
  Finally, let us consider the _Fuel Price_ factor. As shown in the following table, the **RMSE** increases further. In the same time, the prediction line in the mismatch figure does not change much either. 
```{r,  cache=TRUE, warning=FALSE, message = FALSE, comment=NA}
...
legend(1, 2300000, legend=c("Input", "Predicted"),
       col=c("red", "blue"), lty=1:2, cex=0.8)
```
As a summary of the linear model tests, the **RMSE** increased with increase of the complexity of the models, such that the linear regression seems cannot properly model our data. Nevertheless, the **importance** function may give us some hints. As shown below, the importance function shows that _CPI_ is the dominant factor here. The other two have extremely limited contribution. Therefore, we may need look for alternatives.
```{r,  cache=TRUE, warning=FALSE, message = FALSE, comment=NA}
varImp(fit)  
```
...
## Nonlinear Models
  ...
  Since linear modeling has the weakness, I introduce two nonlinear models here. One is  **_K-Nearest Neighbors_** method and the other is  **_Random Forests_** method, plus cross validation.  

#### Model _5_: K-Nearest Neighbors

  **_K-Nearest Neighbors_** method is well known in data analysis. It can be used on both categorical and continuous variables while our data belongs to continuous category. As shown below, I use the three independent variables, **CPI**, **Unemployment** and **Fuel_Price**, simultaneously as the input. In Regard to the **cross validation**, I tune the fold number _k_.  Meanwhile, since we have totally _71_ records in the train set, I choose the maximum _k_ as _71_. 
  
  Now let us look at the result. as shown in the table, the **RMSE** has decreased this time. The fitting curve shows the optimal _k_ as _27_.  Also the prediction line shows more features than the linear models above.    
...
#### Model _6_: Random Forests
 ...
 Let us try another nonlinear method, **_Random Forests**. It also works for categorical and continuous variables. Same as Model 5, the input also contains all the three independent variables together. In regards to **cross validation**, I tune parameter, _mtry_, ranging from _1_ to _7_.
  
  As shown in the table below, the **RMSE** has been reduced below that of Model 5. The fitting curve also shows the optimal _mtry_ as _1_. Looking at the mismatch plot below, the prediction has much more features than all the models above. The result from the importance function below also provides further insight. It shows that _CPI_ is still the most dominant factor, but the weight of the _Fuel_Price_ went up dramatically from the about _5%_ in Model 3 to _75.9%_ here. Consequently, the **Random Forests**, thus, brought us the best fitting model.
...
# Conclusion
  ...
  In conclusion, in this project, I have completed two types of tasks assigned by the author. First, the **Basic statistics tasks** tasks were successfully accomplished in section **2.2** with clear conclusions. 
  
  Second, I have tried **_six_** prediction models were in section **3** using both **linear **and **nonlinear** approaches to address **Statistical Model** issue. The linear model did not work well here. With increased predictors, the **RMSE** did not decrease accordingly. Rather it has been increasing. As alternatives, I tried two more nonlinear models, **K-Nearest Neighbors** and  **Random Forests**, with **cross validation**. It turns out that the nonlinear Models can produce much better fitting models. Their **RMSEs** are smaller than the linear models and **Random Forests** behaves as the best model. It brought a _183330_ **RMSE**. Moreover, I also find that the most important factor is **CPI**. The dominance of the **Fuel_Price** increased after adopting nonlinear method. However, the importance of the **Unemployment** is _0_ for all methods. We may conclude that **Unemployment** has little impact to the **weekly sales** whereas **CPI** and **Fuel_Price** do have wieghted by their dominance. 
  
  Finally, the data mismatch plot of our best model, Model 6, still shows that there are still big mismatch between the data and prediction. Therefore, there must be big room to improve our modeling methods and more study is necessary. For instance, we maybe introduce more input factors, like, Holiday, and try more advanced algorithms, etc. 
  
  Furthermore, the store sales dataset provides lot information and we only tested the data based on Store 1. The other stores' data may give us more insights about the in-store sales. In the future, we may look into other stores.  
